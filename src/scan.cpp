...
int main(int argc, char *argv[])
{
  ...
  try
  {
    ...
    inputs       = (jconf.has_member("inputs"))       ? jconf["inputs"].as<std::vector<std::string>>()       : std::vector<std::string>();
    if (!inputs.size()) throw std::runtime_error("No inputs given");
    ...
    // debug parameters
    out_videofile = inputs.front() + ".tracking.avi";
    out_detfile = inputs.front() + ".det.csv";
    out_imagebase = inputs.front();
  ...
  try
  {
    for(int inputi = 0; inputi < (int) inputs.size(); ++inputi)
    {
      id_box = inputs[inputi] + "-" + id_box.substr(id_box.find_last_of("-") + 1);
      id_box = id_box.substr(id_box.find_last_of("/\\") + 1);
      std::cout << "**** " << inputi <<  " " << id_box << std::endl;
      preview_boxes_t large_preview(100, 150, false), small_preview(50, 50, true);
      bool show_small_boxes = false;

      std::string filename = inputs[inputi];
