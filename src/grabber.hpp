...
private:
  ...
  bool mode_live;
public:
  GrabberThread() : grabOn(false), device(""), bufSizeMax(0), mode_live(true){};
  ~GrabberThread()
  {
    grabOn.store(false);
    cap.release();
  ...
  bool Init(std::string dev = "", bool using_videofile = false)
  {
    device = dev;
    mode_live = !using_videofile;
    cap.open(device);
    return cap.isOpened();
  }
  ...
  void GrabThread()
  {
    ...
    cv::Mat tmp;
    grabOn.store(true);
    while (grabOn.load() == true && mode_live)
    {
