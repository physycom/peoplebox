...
int main(int argc, char *argv[])
{
  ...
  // vars
  std::string names_file, cfg_file, weights_file, cam_L, cam_R;
  ...
  try
  {
      ...
      )
    {
      ...
      cv::Mat cur_frame_L, cur_frame_R, undist_L, undist_R, stitched, mapx, mapy;
      std::vector<cv::Mat> to_stitch;
...
#endif
        throw std::runtime_error("Unable to open input video " + cam_L + " or " + cam_R);
      }
      video_fps = cap_L.get(CV_CAP_PROP_FPS);
      if(cap_R.get(CV_CAP_PROP_FPS) != video_fps) throw std::runtime_error("Cameras don't have same FPS");
      crossing_frame_num = video_fps * crossing_dt;
      dump_rec_num =  int(dump_dt / float(crossing_dt));
      image_frame_num = video_fps * image_dt;
      cap_L.grab();
      cap_R.grab();
      cap_L.retrieve(cur_frame_L);
      cap_R.retrieve(cur_frame_R);
      cv::Size const frame_size = cur_frame_L.size();
      if (cur_frame_R.size() != frame_size) throw std::runtime_error("Cameras frames have different dimensions");

      // rectification and stitching initialization
      cv::fisheye::estimateNewCameraMatrixForUndistortRectify(matK, matD, frame_size, cv::noArray(), newK, balance);
      cv::fisheye::initUndistortRectifyMap(matK, matD, cv::noArray(), newK, frame_size, CV_32F, mapx, mapy);

      cv::remap(cur_frame_L, undist_L, mapx, mapy, cv::INTER_CUBIC);
      cv::remap(cur_frame_R, undist_R, mapx, mapy, cv::INTER_CUBIC);
      to_stitch.push_back(undist_L);
      to_stitch.push_back(undist_R);
      bool try_use_gpu = false; // needs opencv cuda to work
      cv::Ptr<cv::Stitcher> stitcher = cv::Stitcher::create(cv::Stitcher::PANORAMA, try_use_gpu);
      cv::Stitcher::Status status = stitcher->estimateTransform(to_stitch);
      if (status != cv::Stitcher::OK)
      {
        std::cerr << "Can't transform images to stitch, error code = " << int(status) << std::endl;
        throw std::runtime_error("Unable to estimate stitching transformation");
      }
      status = stitcher->composePanorama(to_stitch, stitched);
      if (status != cv::Stitcher::OK)
      {
        std::cerr << "Can't compose images while stitching, error code = " << int(status) << std::endl;
        throw std::runtime_error("Unable to compose panorama image");
      }
      cv::Size const stitched_size = stitched.size();
      std::cout << "Video input: " << cam_L << " (L), " <<  cam_R << " (R), " << "\n stitched size: " << stitched_size << "  FPS: " << video_fps << std::endl;

      if (enable_video_dump)
        out_video.open(out_videofile, CV_FOURCC('M', 'J', 'P', 'G'), std::max(35, video_fps), stitched_size, true);
        //out_video.open(out_videofile, CV_FOURCC('D', 'I', 'V', 'X'), std::max(35, video_fps), stitched_size, true);
      ...
      t_cap = std::thread([&]()
      {
        ...
        do {
          ...
          cap_L.grab();
          cap_R.grab();
          cap_L.retrieve(detection_data.cap_frame_L);
          cap_R.retrieve(detection_data.cap_frame_R);
          fps_cap_counter++;
          ...
          if (detection_data.cap_frame_L.empty() || detection_data.cap_frame_R.empty() || exit_flag) {
            ...
            detection_data.cap_frame_L = cv::Mat(cur_frame_L.size(), CV_8UC3);
            detection_data.cap_frame_R = cv::Mat(cur_frame_R.size(), CV_8UC3);
          }
      ...
      t_prepare = std::thread([&]()
      {
        ...
        do {
          ...
          to_stitch.clear();
          to_stitch.push_back(m1);
          to_stitch.push_back(m2);
          status = stitcher->composePanorama(to_stitch, detection_data.frame_stitched);
          if (status != cv::Stitcher::OK)
          {
            std::cerr << "Can't compose images while stitching, error code = " << int(status) << std::endl;
            throw std::runtime_error("Unable to compose panorama image");
      ...
      do {
        ...
        {
          cv::Mat small_frame;
          cv::resize(draw_frame, small_frame, cv::Size(), 1.0, 1.0);
